...
- **aws\_secret\_access\_key** stores the secret access key to the AWS cluster.
- **aws\_session\_token** stores the aws session token to the AWS cluster. (required when using [temporary AWS credentials](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_use-resources.html))
